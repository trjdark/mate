...
import { getIsComment } from "@redux-actions/teaching/feedBack";
...
class FeedBackCourse extends React.Component<PropsSet, any> {
    ...
    toDetailRoute = (params) => {
        getIsComment({ currentCenterId: User.currentCenterId, id: params.lessonId}).then(res=>{
            if(res.code===0){
                message.warning(res.msg)
            }else{
                params.selectedStuNum > 0 ? history.push(`${TeachingRoutes.随堂反馈测评.link}${CommonUtils.stringify(params)}`) : message.warning('没有可点评的宝宝')
            }
        })
    };
