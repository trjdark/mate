...
class ReserveSubmit extends Component<any, any> {
    private couldSubmit = true;  // 提交按钮是否可用 默认情况下可用
    constructor(props) {
        super(props);
        const {currContract, babyInfo, leadsId, cacheCourseList,promoteType} = CommonUtils.parse(this.props);
     ...
     */
    search = () => {
        const {checkedList, babyInfo, currContract, leadsId, cacheCourseList, promoteType} = this.state;
     ...
     */
    preSubmit = () => {
        const {currContract, selectedCourseNum} = this.state;
        const usableCourseNum = currContract ? currContract.usableCourseNum : 0;
        if ((usableCourseNum !== 0) && (selectedCourseNum > usableCourseNum)) {
            message.error(`当前已选课程累计课时数为${selectedCourseNum}, 超过可选课时数，`, 3);
            return;
        }

        // 可选课时为0时
        if (currContract && usableCourseNum === 0) {
            this.setState({
                noNumVisible: true,
    ...
    handleOk = () => {
        ...
        if (this.state.currContract) {
            ...
            previewBookingSave({
                ...
                // 需要审批提交成功
                this.couldSubmit = true;   // 把变量重新设置为可点击状态
                if (this.state.enablePreviewApproval) {
    ...
    render() {
        const {
            ...
            crumb, indeterminate, checkAll, options, checkedList, weeklist, noNumMessage, selectedCourseNum,
        } = this.state;
        const remainingCourseNum = currContract ? currContract.remainingCourseNum : 0;
        const usableCourseNum = currContract ? currContract.usableCourseNum : 0;
        return (
            <Provider value={}>
                <Fragment>
                                ...
                                render={() => (
                                    <div>
                                        <span style={{marginRight: 30}}>
                                            剩余课时：{remainingCourseNum}
                                        </span>
                                        <span style={{marginRight: 30}}>
                                            已选课时：{remainingCourseNum - usableCourseNum}
                                        </span>
                                        <span style={{marginRight: 30}}>
                                            可选课时：{usableCourseNum}
                                        </span>
