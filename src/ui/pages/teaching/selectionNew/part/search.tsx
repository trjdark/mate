...
class Search extends React.Component<any, any> {
    ...
    componentDidMount() {
        Promise.all([
            ...
            const {oldCourseInfo} = this.props;
            this.setState({
                courseTypeList: courseType,
                allCourseTypeList: courseType
                    .filter(item => oldCourseInfo[0].businessSource === item.businessSource)
    ...
    handleSubmit = (e) => {
        ...
        form.validateFields((err, values) => {
            if (!err) {
                const {promoteType} = values;
                const {leadsId, hasContract, setTableList, oldCourseInfo} = this.props;
                const {checkedList, courseTypeList, contractList} = this.state;
                const selectionTableWrapper:any = document.querySelector('.selection-table-wrapper');   // 课程表
                const gymContentWrap = document.querySelector('#gym-content-wrap > div');           // 内容区的包装元素
                let params = {
                    babyId: oldCourseInfo[0].babyId,
                    contractId: oldCourseInfo[0].contractId,
                    leadsId,
                    promoteType,
                    currentCenterId: User.currentCenterId,
    ...
    * */
    ...
    render() {
        ...
        const {hasContract, oldCourseInfo} = this.props;
        const {
            contractList, indeterminate, checkAll, allCourseTypeList,
            checkedList, changePkgVisible,remainCourseNum
        } = this.state;
            ...
            <div className="selection-search-wrapper shadow">
                <Row className="row-name">
                    <Col span={12} className="name">
                        宝宝姓名：{oldCourseInfo[0].babyName}
                    </Col>
                    <Col span={12} className="month">
                        月龄：{oldCourseInfo[0].monthValue}
                    </Col>
                </Row>
                {/*{*/}
                                ...
                                {/*<FormItem>*/}
                                    {/*{*/}
                                        {/*getFieldDecorator(`contractId`, {*/}
                                            {/*rules: [],*/}
                                            {/*initialValue: oldCourseInfo[0].contractId*/}
                                        {/*})(*/}
                                            {/*<Select style={{ width: 390 }} onChange={(value: string) => this.handleChangePkg(value)} disabled={true}>*/}
                                                {/*{*/}
                            ...
                            {/*() => (*/}
                                {/*<FormItem>*/}
                                    {/*{*/}
                                        ...
                                        {/*})(*/}
                                            {/*<Select style={{width: 300}} onChange={(value:string) => this.handleChangePkg(value)} disabled={true}>*/}
                                                {/*{*/}
                                ...
                                <FormItem>
                                    {
                                        getFieldDecorator(`contractId`, {
                                            ...
                                            <Select style={{width: 300}} onChange={(value:string) => this.handleChangePkg(value)} disabled>
                                                {
                                                    contractList.map(item =>
                                                        <Option value={item.contractId} key={item.contractId}>
                                                            {item.packageName}
                ...
                />
                 {
                    ...
                    <Row className="btn">
                        <Button
                            ...
                            disabled={contractList.length <= 0}>查询</Button>
                    </Row>
                }
                {
                    !hasContract&&
