...
class CourseSelectionNew extends React.Component<any, any> {
    constructor(props) {
        super(props);
        const { hasContract, currBabyInfo, leadsId, exchangeClassNum} = CommonUtils.parse(this.props);
        ...
        this.state = {
            ...
            promoteType: 0,            // 选课类型
            exchangeClassNum: exchangeClassNum, // 可换课课时
            oldCourseInfo: oldCourseInfo,             // 原课程课程日期列表
        }
    }
    ...
    /*跳转只提交选课界面*/
    submitSelection = () => {
        const { cacheCourseList, babyInfo, currContract, leadsId, promoteType, exchangeClassNum} = this.state;
        const params = { cacheCourseList, babyInfo, currContract, leadsId, promoteType, exchangeClassNum};
        // 可以走 saga
        history.push(TeachingRoutes.提交预定新.link + CommonUtils.stringify(params))
    };
    ...
    */
    ...
    render() {
        const { arrange, currCacheCourse, crumb, babyInfo, leadsId, hasContract, tableList, cacheCourseList, exchangeClassNum, oldCourseInfo} = this.state;
        return (
            <Provider
                value={{
            ...
            >
                ...
                <Search
                    ...
                    exchangeClassNum={exchangeClassNum}
                    getPromoteType={this.getPromoteType}
                    oldCourseInfo={oldCourseInfo}
                />
                <CourseTable
                    arrange={arrange}
                    tableList={tableList}
