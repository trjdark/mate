...
import { bulkPrint, changeCheckInTime } from "@redux-actions/teaching/scheduleAction";
import { getFeedCourseList} from "@redux-actions/teaching/feedBack";
...
import {  getIsCommentNew } from "@redux-actions/teaching/feedBack";
...
class FeedBackCourse extends React.Component<PropsSet, any> {
    colorChoice = {
        lightBlue: '#E3F9FF',
        grey: '#EBEFF3'
    };
    ...
    toDetailRoute = (params) => {
        getIsCommentNew({ currentCenterId: User.currentCenterId, id: params.lessonId}).then(res=>{
            if(res.code===0){
                message.warning(res.msg)
            }else{
                params.selectedStuNum > 0 ? history.push(`${TeachingRoutes.随堂反馈测评新.link}${CommonUtils.stringify(params)}`) : message.warning('没有可点评的宝宝')
            }
        })
    };
    ...
    render() {
        ...
        return (
            <div className='bgWhite gym-teaching-schedule-signin bgWhite page-wrap'>
