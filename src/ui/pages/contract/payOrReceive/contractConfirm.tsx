...
import {moneyType_format} from "../filter/contractFilter";
...
import {contract_type__format} from "../filter/contractFilter";
...
class ContractConfirm extends React.Component<any, any> {
    ...
    handleOk = () => {
        ...
        if(window.location.href.indexOf('confirmReceive') > -1){
            if(this.state.ifChecked){
                if(this.state.amountInfo.earnestList[0]){
                    postParams.financialRecordIdList.push(this.state.amountInfo.earnestList[0].id);
                }else{
                    postParams.financialRecordIdList = null
                }
            }else{
         ...
         */
        if(window.location.href.indexOf('confirmReceive') > -1){
            confirmReceive(postParams, this.submitRequestId).then((res) => {
                message.success('收款成功!');
                // history.push(`${Routes.合同收款管理合同.path}`)
                history.push(`${Routes.合同收款管理.link}${CommonUtils.stringify({type: 'contract'})}`)
            }, (err) => {
                //返回请求reject
                message.error(err.msg);
            })
        }else{
            confirmPay(payPostParams, this.submitRequestId).then((res) => {
                message.success('付款成功!');
                // history.push(Routes.合同付款管理合同.path)
                history.push(`${Routes.合同付款管理.link}${CommonUtils.stringify({type: 'contract'})}`)
        ...
        return (
            <div id='gym-contract-receive'>
                                ...
                                <div>
                                    ...
                                    <Row className='gym-contract-table-tbody'>
                                        ...
                                        <Col span={8}  className='gym-contract-table-tbody-value'>
                                            <span>
                                                {
                                                    businessSourceMap.filter((item:any) => item.businessSourceCode === basicInfo.businessSource).length > 0
                                                    ? businessSourceMap.filter((item:any) => item.businessSourceCode === basicInfo.businessSource)[0].businessSourceValue
                                                    : '-'
                                                }
                                            </span>
                                        </Col>
                                        {
                                            basicInfo.businessSource === "75002" &&
                                            <Col span={4}  className='gym-contract-table-tbody-label'>
                                                <span>关联合同:</span>
                                            </Col>
                                        }
                                        {
                                            <Col span={8}  className='gym-contract-table-tbody-value'>
                                                {
                                                    (basicInfo.relatedContracts || []).map((item:any, index:number) =>
                                ...
                                <Row className='gym-contract-table-tbody gym-contract-table-tbody-left gym-contract-table-tbody-right'>
                                            ...
                                            <FormItem className='gym-act-modal-form'>
                                                {
                                                    getFieldDecorator('receiveDate', {
                                                       ...
                                                       <span>
                                                           {amountInfo.lastUpdateDate !== '' ?moment(amountInfo.payDate).format('YYYY-MM-DD'):''}
                                                       </span>
                                                    )
                                                }
                                            </FormItem>
                                    ...
                                    <Col span={8} className='gym-contract-table-tbody-form gym-contract-table-tbody-right'>
                                        <FormItem className='gym-act-modal-form'>
                                            {
                                                amountInfo.hasPayment === 1 &&
                                                `${amountInfo.operatorStaffName ? amountInfo.operatorStaffName :''}`
                                            }
                                            {
                                                amountInfo.hasPayment === 0 &&
                                        ...
                                        <div>
                                                            ...
                                                            <div>
                                                                <span className='gym-contract-confirm-check-label'>
                                                                    ...
                                                                    {amountInfo.earnestList.length > 0 ? amountInfo.earnestList[0].amount : 0}
                                                                     ...
                                                                     {`(${ amountInfo.earnestList[0] ? moment(amountInfo.earnestList[0].time).format('YYYY-MM-DD') : ''})`}
                                                                </span>
                                                            </div>
                                                            <div>
                                            ...
                                            <Row>
                                                <Col span={24}>
                                                    {
                                                        matchReceive &&
                                                        <CancelButton submitText='收款' form={form} goBackLink={`${Routes.合同收款管理.link}${CommonUtils.stringify({type: 'contract'})}`}/>
                                                    }
                                                    {
                                                        matchPay &&
                                                        <CancelButton submitText='付款' form={form} goBackLink={`${Routes.合同付款管理.link}${CommonUtils.stringify({type: 'contract'})}`}/>
                                                    }
                                                </Col>
                                            </Row>
                                        </div>
                                        ...
                                        <div>
                                            <Row className='gym-contract-confirm-content'>
                                                <Col span={11}>
                                                    ...
                                                    <span className='gym-contract-confirm-value'>{(amountInfo.earnestList&&amountInfo.earnestList.length > 0) ? amountInfo.actualAmount.toFixed(2): amountInfo.actualAmount.toFixed(2)}</span>
                                                </Col>
                                                <Col span={12}>
                                                    {
                                                        matchReceive &&
                                                        <span className='gym-contract-confirm-label'>抵扣会员定金:</span>
                                                    }
                                                    {
                                                        matchPay &&
                                                        <span className='gym-contract-confirm-label'>抵扣会员定金:</span>
                                                    }
                                                    <span className='gym-contract-confirm-value'>{(amountInfo.earnestList&&amountInfo.earnestList.length > 0) ? amountInfo.earnestList[0].amount.toFixed(2): 0.00}</span>
                                                    {
                                                        (amountInfo.earnestList && amountInfo.earnestList.length > 0) &&
                                                        <span className='gym-contract-confirm-date'>({moment(amountInfo.earnestList[0].time).format('YYYY-MM-DD')})</span>
                                                    }
                                                </Col>
                                            </Row>
                                            <Row>
                ...
                <Modal
                    ...
                    contentText={`确定要给${this.lengthFormat(basicInfo.babyName)}进行${window.location.href.indexOf('confirmReceive') > -1? `新建合同的收款吗？`:'退课的付款吗？'}`}
                />
            </div>
        )
    }
