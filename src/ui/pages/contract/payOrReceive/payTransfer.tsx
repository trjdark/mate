...
class ContractActionPayTransfer extends React.Component<any, any> {
    //路由代码块
    ...
    checkStatus = (record:any) => {
        ...
        getContractDetailAmount(postData).then((res) => {
            if(res.hasPayment === record.hasPayment){
                history.push(`${Routes.确认改中心付款.link}${CommonUtils.stringify({
                    aparId:record.aparId,
                    leadsId:record.leadsId,
    ...
    transferOutRetract = (record:any) => {
        ...
        transferOutRetractChangeCenter(param).then( () => {
                    ...
                    <div>
                        {
                            ...
                            <button onClick={() => {this.checkStatus(record)}} className='gym-button-xxs gym-button-white'>付款</button>
                        }
                        {
                            (record.approvalStatus === '1205003' && User.permissionList.includes(FUNC['转中心付款退回'])) &&
                            <ConfirmCheck
                                item={record}
                                ensure={() => this.transferOutRetract(record)}
...
export {ContractActionPayTransfer}
