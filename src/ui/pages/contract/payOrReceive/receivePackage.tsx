...
const searchConfigCommon:Array<any> = [
    {
        label: '宝宝姓名',
        required: false,
        type: 'text',
...
class ContractActionReceivePackage extends React.Component<any, any> {
    //路由代码块
    ...
    checkStatus = (record:any) => {
        ...
        getContractDetailAmount(postData).then((res) => {
            if(res.hasPayment === record.hasPayment){
                history.push(`${Routes.确认改包收款.link}${CommonUtils.stringify({
                    aparId:record.aparId,
                    leadsId:record.leadsId,
...
export {ContractActionReceivePackage}
