...
class PackageConfirm extends React.Component<any, any> {
    ...
    handleOk = () => {
        ...
        if(window.location.href.indexOf('confirmReceive') > -1){
            confirmReceive(postParams, this.submitRequestId).then(() => {
                ...
                history.push(`${Routes.合同收款管理.link}${CommonUtils.stringify({type: 'package'})}`)
            ...
            confirmPay(payPostParams, this.submitRequestId).then(() => {
                ...
                history.push(`${Routes.合同付款管理.link}${CommonUtils.stringify({type: 'package'})}`)
        ...
        return (
            <div id={`gym-contract-receive`}>
                                        ...
                                        <div>
                                            ...
                                            <Row>
                                                <Col span={24}>
                                                    {
                                                        matchReceive &&
                                                        <CancelButton submitText='收款' form={form} goBackLink={`${Routes.合同收款管理.link}${CommonUtils.stringify({type: 'package'})}`}/>
                                                    }
                                                    {
                                                        matchPay &&
                                                        <CancelButton submitText='付款' form={form} goBackLink={`${Routes.合同付款管理.link}${CommonUtils.stringify({type: 'package'})}`}/>
                                                    }
                                                </Col>
                                            </Row>
                                        </div>
                ...
                <Modal
                    ...
                    contentText={`确定要给${this.lengthFormat(basicInfo.babyName)}进行${matchReceive?'改包的收款吗？':'改包的付款吗？'}`}
                />
            </div>
        )
    }
